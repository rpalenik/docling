#!/usr/bin/env python3
"""
Docling parser pre docling dokumenty - založený na prístupe docling chunkeru.

Tento modul implementuje rekonštrukciu celého docling dokumentu
do hierarchickej JSON štruktúry pomocou doc.iterate_items().

Vlastnosti:
- Používa doc.iterate_items(with_groups=True) na iteráciu cez všetky položky
- Tabuľky a obrázky sa objavujú priamo v hierarchii na správnom mieste
...
def reconstruct_document(doc: DoclingDocument) -> Dict[str, Any]:
    """
    Reconstruct entire document using doc.iterate_items() approach.
    
    This function processes all document items (text, tables, pictures) in hierarchical
    order using doc.iterate_items(), building the complete hierarchical structure as it
    encounters markers. Tables and pictures appear naturally at their correct positions.
    ...
    log_progress("INFO", "Starting document reconstruction using iterate_items()...")
    ...
    structure = {
        ...
        "metadata": {
            ...
            "reconstruction_method": "iterate_items",
    ...
    # Track item count for progress logging
    item_count = 0
    last_log_time = time.time()
    log_interval = 2.0  # Log every 2 seconds
    
    # Process each item in hierarchical order using iterate_items()
    log_progress("INFO", "Processing document items using iterate_items(with_groups=True)...")
    ...
    for item, level in doc.iterate_items(with_groups=True):
        ...
        if current_time - last_log_time >= log_interval:
            log_progress("INFO", f"Processing item {item_count:,} (level {level})...")
            last_log_time = current_time
        
        # Check for page header/footer labels (structural identification)
        # Skip page headers and footers - they are not document content
        # Note: Not all documents have page headers/footers properly labeled,
        # so we also use pattern matching as fallback in footnotes section
        label = getattr(item, 'label', None)
        if label in [DocItemLabel.PAGE_HEADER, DocItemLabel.PAGE_FOOTER]:
            continue  # Skip this item entirely
        
        # Handle different item types
        # 1. TableItem - assign to current context
        if isinstance(item, TableItem):
            # Find table index in doc.tables
            table_idx = None
            if hasattr(doc, 'tables'):
                for idx, table in enumerate(doc.tables):
                    if table is item or (hasattr(table, 'self_ref') and hasattr(item, 'self_ref') and 
                                         getattr(table, 'self_ref', None) == getattr(item, 'self_ref', None)):
                        table_idx = idx
                        break
            
            if table_idx is not None:
                # Format table with full data
                table_data = format_table_for_json(item, doc, table_idx)
                
                # Assign to current context
                if in_annex_section and current_annex:
                    # Add to annexes section
                    if table_idx not in seen_annex_tables:
                        structure["annexes"]["tables"].append(table_data)
                        seen_annex_tables.add(table_idx)
                        log_progress("DEBUG", f"Found table {table_idx} in annex section")
                elif current_pismeno:
                    # Add to current pismeno
                    if current_pismeno.get('_current_subitem'):
                        # Add to subitem
                        if 'tables' not in current_pismeno.get('_subitem_texts', [{}])[-1]:
                            current_pismeno['_subitem_texts'][-1]['table_idx'] = table_idx
                    else:
                        # Add to pismeno
                        if not any(t.get('table_idx') == table_idx for t in pismeno_texts):
                        ...
                        log_progress("DEBUG", f"Found table {table_idx} in pismeno {current_pismeno.get('id', 'unknown')}")
                elif current_odsek:
                    # Add to current odsek
                    if not any(t.get('table_idx') == table_idx for t in odsek_texts):
                        odsek_texts.append({
                            "text": "",
                            "raw_text": "",
                            "table_idx": table_idx,
                        ...
                        log_progress("DEBUG", f"Found table {table_idx} in odsek {current_odsek.get('id', 'unknown')}")
                elif current_paragraph:
                    # Add to paragraph (will be processed in _close_paragraph)
                    if 'tables' not in current_paragraph:
                        current_paragraph['tables'] = []
                    if not any(t.get('index') == table_idx for t in current_paragraph['tables']):
                        current_paragraph['tables'].append({"index": table_idx})
                        log_progress("DEBUG", f"Found table {table_idx} in paragraph {current_paragraph.get('id', 'unknown')}")
            continue
        
        # 2. PictureItem - assign to current context
        if isinstance(item, PictureItem):
            # Find picture index in doc.pictures
            picture_idx = None
            if hasattr(doc, 'pictures'):
                for idx, picture in enumerate(doc.pictures):
                    if picture is item or (hasattr(picture, 'self_ref') and hasattr(item, 'self_ref') and 
                                           getattr(picture, 'self_ref', None) == getattr(item, 'self_ref', None)):
                        picture_idx = idx
                        break
            ...
            if picture_idx is not None:
                ...
                if in_annex_section and current_annex:
                    ...
                    structure["annexes"]["pictures"].append({
                        "index": picture_idx,
                        "position_in_text": len(structure["annexes"]["content"])
                    })
                    log_progress("DEBUG", f"Found picture {picture_idx} in annex section")
                elif current_pismeno:
                    # Add to current pismeno
                    if current_pismeno.get('_current_subitem'):
                        # Add to subitem
                        if 'pictures' not in current_pismeno.get('_subitem_texts', [{}])[-1]:
                            current_pismeno['_subitem_texts'][-1]['picture_idx'] = picture_idx
                    else:
                        # Add to pismeno
                        if not any(t.get('picture_idx') == picture_idx for t in pismeno_texts):
                            pismeno_texts.append({
                                "text": "",
                                "raw_text": "",
                                "table_idx": None,
                    ...
                    # Add to current odsek
                    if not any(t.get('picture_idx') == picture_idx for t in odsek_texts):
                        odsek_texts.append({
                            "text": "",
                            "raw_text": "",
                            "table_idx": None,
                    ...
                    # Add to paragraph
                    if 'pictures' not in current_paragraph:
                        current_paragraph['pictures'] = []
                    if not any(p.get('index') == picture_idx for p in current_paragraph.get('pictures', [])):
                        current_paragraph['pictures'].append({"index": picture_idx})
            continue
        
        # 3. SectionHeaderItem or TitleItem - handle headers (usually part markers)
        if isinstance(item, (SectionHeaderItem, TitleItem)):
            ...
            # Check for footnotes section start FIRST (before part marker check)
            if detect_footnotes_section(normalized_text, hyperlink):
                in_footnotes_section = True
                log_progress("INFO", f"Found footnotes section at item {item_count}")
                continue
        ...
        # 4. TextItem - process as before (marker detection, etc.)
        if not isinstance(item, TextItem):
            # Skip other item types (GroupItem, etc.) for now
            continue
        
        # Get text content from TextItem
        text_element = item
        ...
        # NOTE: Footnotes section detection is already handled for SectionHeaderItem above
        # Only check here if "Poznámky" appears as TextItem (fallback)
        if not in_footnotes_section and detect_footnotes_section(normalized_text, hyperlink):
            in_footnotes_section = True
            log_progress("INFO", f"Found footnotes section at item {item_count}")
            continue
        ...
        if in_footnotes_section:
            ...
            if label == DocItemLabel.PAGE_FOOTER:
                ...
                in_footnotes_section = False
                log_progress("INFO", f"Detected page footer (label={label}) at item {item_count}, ending footnotes section")
            ...
            if content_layer == ContentLayer.FURNITURE:
                ...
                in_footnotes_section = False
                log_progress("INFO", f"Detected furniture layer (footer) at item {item_count}, ending footnotes section")
                continue  # Skip footer content
            
            # Fallback: Filter out UI elements and navigation text (pattern matching)
            # This is less reliable but catches cases where label might not be set correctly
            ...
            # Check for footnote marker FIRST
        ...
        if in_annex_section:
            ...
            if annex_num:
                ...
                if current_annex:
                    _close_annex(current_annex, annex_texts, doc, structure, None, None, annex_marker_text_elem, all_annex_texts, seen_annex_tables)
                ...
                annex_texts = []
                log_progress("INFO", f"Found annex: {annex_num} at item {item_count}")
                continue
        ...
        # Debug logging for first 100 items
        if item_count <= 100:
            log_progress("DEBUG", f"Item {item_count} (level {level}): text='{normalized_text[:80]}' "
        ...
        if para_num and not has_hyperlink:
            ...
            log_progress("DEBUG", f"Found paragraph: § {para_num} at item {item_count}")
            continue
        ...
        if pismeno_letter:
            ...
            # Note: We can't easily get previous element in iterate_items(), 
            # but previous_elements from stored data should be sufficient
            ...
            if is_reference:
                ...
                # Get current item info for reference
                element_info = f"item={item_count}"
    ...
    # Close last footnote if any (in case footnotes section ended without closing)
    if current_footnote:
        _close_footnote(current_footnote, footnote_texts, structure)
        footnotes_count += 1
...
def _close_annex(annex: Optional[Dict], texts: List[Dict], doc: DoclingDocument, structure: Dict, annex_start_idx: Optional[int] = None, annex_end_idx: Optional[int] = None, annex_marker_text_elem=None, all_annex_texts: List = None, seen_annex_tables: set = None) -> None:
    ...
    Args:
        ...
        annex_start_idx: Not used (kept for compatibility)
        annex_end_idx: Not used (kept for compatibility)
        annex_marker_text_elem: Not used (kept for compatibility)
    ...
    # Note: With iterate_items() approach, tables appear naturally in iteration
    # so we don't need to search for them separately. Tables are already assigned
    # when encountered during iteration.
...
def _close_paragraph(paragraph: Optional[Dict], intro_texts: List[Dict], part: Optional[Dict], doc: DoclingDocument) -> None:
    ...
    # Tables are already assigned during iteration, so we just need to clean up
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Docling document parser using iterate_items() approach')
    ...
    # Set up log file
    log_file_path = output_dir / f"{base_name}_docling.log"
    ...
    try:
        ...
        log_progress("INFO", "Docling Document Parser (iterate_items approach)")
        log_progress("INFO", "=" * 70)
        ...
        json_output = output_dir / f"{base_name}_docling.json"
        md_output = output_dir / f"{base_name}_docling.md"
